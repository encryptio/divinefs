...
uint8_t read_uint8(exec_options *eo, off_t offset) {
    uint8_t buf;
    EO_READ(eo, &buf, offset, 1);
    return buf;
}

int8_t read_int8(exec_options *eo, off_t offset) { // 2s complement
    uint8_t buf;
    EO_READ(eo, &buf, offset, 1);
    int8_t ret = buf & 0x7F;
    if ( buf & 0x80 )
